...
export function UncertaintyColumns(
  editingCell,
  setEditingCell,
  cellValue,
  setCellValue,
  ...
  return [
    ...
    {
      accessorKey: "keyUncertainty",
      header: "Key Uncertainty",
      size: 150,
      cell: (info) => {
        const cellId = `${info.row.original.uuid}_keyUncertainty`;
        return editingCell === cellId ? (
          <div>
            <Autocomplete
              value={
                cellValue[cellId] || (info.getValue() === "true" ? "Key" : null)
              }
              options={["Key", "Unset"]}
              onOptionsChange={(e) => {
    ...
    },
    {
      ...
      cell: (info) => {
        const cellId = `${info.row.original.uuid}_outcomes`;
        const currentValue = cellValue[cellId] || info.row.getValue("outcomes");
        return editingCell === cellId ? (
          <TextField
            value={currentValue}
            onChange={(e) => handleLabelChange(e, cellId)}
          ...
          <div onClick={(e) => setEditingCell(cellId)}>
            <ul>
              {currentValue?.map((outcomes, index) => (
