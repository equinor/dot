...
def test_header_prefix_and_data():
    assert markdown_elements.header(
        2,
        "Title",
        "Once upon a time"
...
def test_text_line():
    assert markdown_elements.text_line(
        "A text to be written"
        ) == "A text to be written \n"
    

def test_item_line_unordered():
    assert markdown_elements.item_line(
        "A text to be written",
        2
        ) == "    - A text to be written \n"
...
def test_unordered_list():
    assert markdown_elements.unordered_list(
        ["A text to be written",
         "And a second item"],
        ) == "  - A text to be written \n  - And a second item \n\n"        
...
def test_ordered_list():
    assert markdown_elements.ordered_list(
        ["A text to be written",
         "And a second item"],
        ) == "  1. A text to be written \n  1. And a second item \n\n"    
...
def test_two_columns_table_row():
    assert markdown_elements.two_columns_table_row("First", "Second") == \
        "| First | Second |\n"
    assert markdown_elements.two_columns_table_row("First", ["Second", "Third"]) == \
        "| First | Second; Third |\n"
    assert markdown_elements.two_columns_table_row("First", ["Second"]) == \
        "| First | Second |\n"
    assert markdown_elements.two_columns_table_row("First", []) == \
        "| First | - |\n"
    assert markdown_elements.two_columns_table_row("First", [None]) == \
        "| First | - |\n"
    assert markdown_elements.two_columns_table_row("First", None) == \
        "| First | - |\n"
    assert markdown_elements.two_columns_table_row("First", "") == \
